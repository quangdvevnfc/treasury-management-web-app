...
const FilterForm = forwardRef(({onSubmitComplete}, ref) => {
  ...
  const sortedColumn = useSelector(
    (state) => state.customerList.state.sortedColumn
  );

  const handleSubmit = (values) => {
    onSubmitComplete?.();
    dispatch(setFilterData(values));
    dispatch(getCustomers(initialTableData));
      ...
      {({values, touched, errors}) => (
        <Form>
          <FormContainer>
            ...
            >
              <h6 className="mb-4">Customer Category</h6>
              <Field name="category">
                {({field, form}) => (
                  <>
                    <Checkbox.Group
            ...
            <FormItem
              invalid={errors.customerStatus && touched.customerStatus}
              errorMessage={errors.customerStatus}
            >
              <h6 className="mb-4">Customer Status</h6>
